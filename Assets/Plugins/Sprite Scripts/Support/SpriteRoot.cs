...
public struct Rect3D
{
	...
	{
		Vector3 vTmpTl = matrix.MultiplyPoint3x4(rect.m_tl);
		Vector3 vTmpTr = matrix.MultiplyPoint3x4(rect.m_tr);
		Vector3 vTmpBl = matrix.MultiplyPoint3x4(rect.m_bl);

		float fXMin = Mathf.Min(Mathf.Min(vTmpBl.x, vTmpTl.x), vTmpTr.x);
		float fXMax = Mathf.Max(Mathf.Max(vTmpBl.x, vTmpTl.x), vTmpTr.x);
		float fYMin = Mathf.Min(Mathf.Min(vTmpBl.y, vTmpTl.y), vTmpTr.y);
		float fYMax = Mathf.Max(Mathf.Max(vTmpBl.y, vTmpTl.y), vTmpTr.y);

		vTmpTl = new Vector3(fXMin, fYMax, vTmpTl.z);
		vTmpTr = new Vector3(fXMax, fYMax, vTmpTl.z);
		vTmpBl = new Vector3(fXMin, fYMin, vTmpTl.z);

		return new Rect3D(vTmpTl, vTmpTr, vTmpBl);
	}
}
...
// 			return true;
	...
	public virtual Rect3D ClippingRect
	{
		...
		set
		{
			...
			// Verify that we are actually being clipped:
			if (leftClipPct != 1f ||
				rightClipPct != 1f ||
				topClipPct != 1f ||
				bottomClipPct != 1f)
