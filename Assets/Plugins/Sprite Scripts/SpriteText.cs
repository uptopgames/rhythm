				...
				for(int i=0; i<str.Length; ++i)
				{
					...
					if (spaceLeft < 0)
					{
						if (i == lastWhiteSpace)
							...
							spaceLeft = ((maxWidthInPixels) ? (maxWidth * worldUnitsPerScreenPixel) : maxWidth) -spriteFont.GetWidth(displaySB, lastWhiteSpace + 1, i, characterSpacing) * worldUnitsPerTexel;
				...
				if(maxWidth > 0 && multiline)
				{
					...
					if (spaceLeft < 0)
					{
						...
						if (strOffset == lastPlainWhiteSpace)
							...
							spaceLeft = ((maxWidthInPixels) ? (maxWidth * worldUnitsPerScreenPixel) : maxWidth) - spriteFont.GetWidth(displaySB, lastWhiteSpace + 1, strOffset, characterSpacing) * worldUnitsPerTexel;
...
*/
	...
	protected void DoSingleLineTruncation()
	{
		...
		if (maxWidth > 0)
		{
			float width = spriteFont.GetWidth(displaySB, 0, displaySB.Length - 1, characterSpacing) * worldUnitsPerTexel;
			float maximumWidth = (maxWidthInPixels) ? (maxWidth * worldUnitsPerScreenPixel) : maxWidth;

			if (width > maximumWidth)
			{
				int clipCount = 0;
				float ellipsisWidth = spriteFont.GetWidth("...", characterSpacing) * worldUnitsPerTexel;

				// Find out how many characters we need to clip:
				do
				{
					++clipCount;
					width = spriteFont.GetWidth(displaySB, 0, displaySB.Length - 1 - clipCount, characterSpacing) * worldUnitsPerTexel;
				}
...
// 					lineStart = curNewLine + 1;
	...
	protected void Layout_Single_Line()
	{
		...
		if (displayString.Length == 1)
			width = spriteFont.GetWidth(displayString, characterSpacing) * worldUnitsPerTexel;
		else
			width = spriteFont.GetWidth(displayString, characterSpacing) * worldUnitsPerTexel;// *characterSpacing;
	...
	protected void Layout_Multiline(string[] lines)
	{
		...
		{
			widths[i] = spriteFont.GetWidth(lines[i], characterSpacing) * worldUnitsPerTexel;
	...
	protected void Layout_Line(Vector3 startPos, string txt, int charIdx)
	{
		...
		if (ch != null)
		{
			pos = startPos + new Vector3(0, ch.yOffset * worldUnitsPerTexel, 0);
			startPos -= new Vector3(ch.xOffset * worldUnitsPerTexel, 0, 0); // Adjust for the first character's xOffset so we actually start where the first character's mesh starts.
		...
		{
			if (ch != null)
				startPos.x += ch.xAdvance * worldUnitsPerTexel * dxCoef;
			...
			startPos.x += ch.GetKerning(txt[i - 1]) * worldUnitsPerTexel * dxCoef;
...
//			gameObject.active = ((Component)parentControl).gameObject.active;
	...
	/// <summary>
	/// Sets the maximum width, of the text.  See the maxWidth member for more details.
	/// The maxWidthInPixels flag controls whether this setting is interpreted as being
	/// specified in pixels or worldspace units.
	/// </summary>
	...
	public Vector3 GetInsertionPointPos(int charIndex)
	{
		...
		charX = spriteFont.GetWidth(displayString, lineStart, charIndex - leftSideOffset, characterSpacing) * worldUnitsPerTexel;
		...
		switch(anchor)
		{
			...
			case Anchor_Pos.Lower_Center:
				...
				else if(alignment == Alignment_Type.Right)
				{
					...
					charX -= spriteFont.GetWidth(displayString, lineStart, lineEnd, characterSpacing) * worldUnitsPerTexel * 0.5f;
				break;
			...
			case Anchor_Pos.Lower_Left:
				...
				{
					charX += (totalWidth * 0.5f) - (spriteFont.GetWidth(displayString, lineStart, lineEnd, characterSpacing) * worldUnitsPerTexel * 0.5f);
				}
				else if(alignment == Alignment_Type.Right)
				{
					charX += totalWidth - (spriteFont.GetWidth(displayString, lineStart, lineEnd, characterSpacing) * worldUnitsPerTexel);
				}
				else
				{
					// Do nothing...
			...
			case Anchor_Pos.Lower_Right:
				...
				{
					charX += -1f * spriteFont.GetWidth(displayString, lineStart, lineEnd, characterSpacing) * worldUnitsPerTexel;
				}
				break;
		}
	...
	public float GetWidth(string s)
	{
		...
		return spriteFont.GetWidth(s) * worldUnitsPerTexel;
	}
